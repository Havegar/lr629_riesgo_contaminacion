...
spark.read.options(header=True,inferSchema=True,delimiter=",").csv("data/processed/fuerza_representante.csv").createOrReplaceTempView("fuerza")
...
contaminados_total.to_csv('data/processed/contaminados_processed_representante.csv', index=False)
...
#   spark.read.parquet("abfs://data@datalakesii.dfs.core.windows.net/DatoOrigen/lr-629/riesgo_contaminacion").createOrReplaceTempView("contaminados")
    spark.read.options(header=True,inferSchema=True,delimiter=",").csv("data/processed/contaminados_processed_representante.csv").createOrReplaceTempView("contaminados")
...
#   raw_path='abfs://data@datalakesii.dfs.core.windows.net/DatosOrigen/lr-629/propuesta_f29_comunidades/contaminacion_representante'
#   contaminados.write.format("parquet").mode('overwrite').save(raw_path) 
    contaminados=contaminados.toPandas()
    contaminados.to_csv('data/processed/contaminados_processed_representante.csv', index=False)
...
spark.sql("select * from contaminados ").toPandas().to_csv('data/processed/contaminados_processed_representante_total.csv', index=False)
