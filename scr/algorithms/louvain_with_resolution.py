...
# Cargar datos desde el archivo CSV de la combiancion de ambas fuerzas
...
# Definir los valores del hiperparámetro de resolución que quieres explorar
...
for resolucion in valores_resolucion:
    ...
    for idx, comunidad in enumerate(louvain_result.communities):
        for node in comunidad:
            df_comunidades = df_comunidades.append({'cont_rut': node, f'comunidad_{resolucion}': idx+1, f'modularidad_{resolucion}': modularity_value}, ignore_index=True)
...
df_final.to_csv(os.path.join(output_directory, 'contaminados_resolution_study.csv'), index=False)